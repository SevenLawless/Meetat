...
**Runtime Configuration:** The frontend now supports runtime configuration via `public/config.js`. During build, this file is automatically generated from your `VITE_API_URL` and `VITE_WS_URL` environment variables. This means:
- ✅ The API URL is embedded at build time from Railway environment variables
- ✅ You can manually edit `public/config.js` after deployment if needed (no rebuild required)
- ✅ The app checks runtime config first, then falls back to build-time config
- ✅ See `public/config.template.js` for configuration options
...
**Runtime Configuration:** The frontend automatically generates `public/config.js` from these environment variables during build. This file is loaded at runtime and takes priority over build-time configuration. You can manually edit `public/config.js` after deployment if needed without rebuilding. See `public/config.template.js` for more details.
...
- **Solution:** Verify `VITE_API_URL` is set correctly in Railway environment variables
- Check that backend is deployed and running
- Verify CORS is configured in backend
- **Runtime Config:** The app uses runtime configuration from `public/config.js`. This file is auto-generated during build from `VITE_API_URL`. If you see "localhost" in errors, the build may have used the wrong environment variable. Check that `VITE_API_URL` is set before building.
